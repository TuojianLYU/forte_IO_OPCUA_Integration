...
EComResponse CBaseCommFB::openConnection() {
  ...
  DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::openConnection()------------------\n");
  DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::openConnection() ID().value = %s------------------\n", PARAMS().getValue());
  EComResponse retVal;
  if (0 == m_poTopOfComStack) {
    // Get the ID
    char *commID;
    if (0 == strchr(PARAMS().getValue(), ']')) {
      commID = getDefaultIDString(PARAMS().getValue());
    }
    else {
      size_t commIdLength = strlen(PARAMS().getValue()) + 1;
      commID = new char[commIdLength];
      memcpy(commID, PARAMS().getValue(), commIdLength);
      commID[commIdLength - 1] = '\0';
    }

    // If the ID is empty return an error
    if ('\0' == *commID) {
      DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::openConnection() ID is empty------------------\n");
      retVal = e_InitInvalidId;
    }
    else {
      DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::openConnection() go to createComstack(commID)------------------\n");
      DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::openConnection() commID = %s------------------\n", commID);
...
EComResponse CBaseCommFB::createComstack(char *commID) {
  DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::createComstack------------------\n");
  ...
  while('\0' != *commID) { // Loop until reaching the end of the ID
    ...
    // Create the new layer
      ...
      DEVLOG_DEBUG("----------------basecommfb CComLayersManager::createCommunicationLayer------------------\n");
    ...
    DEVLOG_DEBUG("----------------basecommfb CComLayersManager::createCommunicationLayer failed------------------\n");
      break;
    }
  }
  return retVal;
...
char *CBaseCommFB::extractLayerIdAndParams(char **paRemainingID, char **paLayerParams) {
  DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::extractLayerIdAndParams------------------\n");
  char *layerID = 0;
  if('\0' != **paRemainingID) {
    char *possibleLayerId = *paRemainingID;
    DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::extractLayerIdAndParams lookForNonEscapedChar------------------\n");
  ...
  DEVLOG_DEBUG("----------------basecommfb CBaseCommFB::extractLayerIdAndParams layerID = %s------------------\n", layerID);
  return layerID;
}
