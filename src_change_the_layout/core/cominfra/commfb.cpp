...
const CStringDictionary::TStringId CCommFB::scm_aunResponderEventInputNameIds[2] = { g_nStringIdINIT, g_nStringIdREQ };
const CStringDictionary::TStringId CCommFB::scm_aunResponderEventOutputNameIds[3] = { g_nStringIdINITO, g_nStringIdIND, g_nStringIdCNF };

const TForteInt16 CCommFB::scm_anEIWithIndexes[] = { 0, 3 };
const TForteInt16 CCommFB::scm_anEOWithIndexes[] = { 0, 3, 7, -1 };
...
void CCommFB::executeEvent(int paEIID) {
  ...
  DEVLOG_DEBUG("----------------commfb md executEvent------------------\n");

  switch (paEIID) {
  case scm_nEventINITID:
    if (true == QI()) {
      DEVLOG_DEBUG("----------------commfb case openConnection------------------\n");
      resp = openConnection();
  ...
  case scm_nSendNotificationEventID:
    DEVLOG_DEBUG("----------------commfb case sendData------------------\n");
    resp = sendData();
    
    break;
  case cg_nExternalEventID:
    DEVLOG_DEBUG("----------------commfb case receiveData------------------\n");
    resp = receiveData();
  ...
  if (e_Nothing != resp) {
    ...
    if (scg_unINIT & resp) {
      DEVLOG_DEBUG("----------------commfb sendOutputEvent(scm_nEventINITOID)------------------\n");
      sendOutputEvent(scm_nEventINITOID);
    }
    else {
      DEVLOG_DEBUG("----------------commfb sendOutputEvent(scm_nReceiveNotificationEventID)------------------\n");
...
EComResponse CCommFB::sendData() {
  DEVLOG_DEBUG("----------------EComResponse CCommFB::sendData()------------------\n");
  EComResponse resp = e_Nothing;
  if (true == QI()) {
...
bool CCommFB::createInterfaceSpec(const char* paConfigString, SFBInterfaceSpec& paInterfaceSpec) {
  DEVLOG_DEBUG("----------------CCommFB::createInterfaceSpec------------------\n");
...
void CCommFB::configureDIs(const char* paDIConfigString, SFBInterfaceSpec& paInterfaceSpec) const {
  DEVLOG_DEBUG("----------------CCommFB::configureDIs------------------\n");
...
void CCommFB::configureDOs(const char* paDOConfigString, SFBInterfaceSpec& paInterfaceSpec) const {
  ...
  if(forte::com_infra::e_DataOutputs == (forte::com_infra::e_DataOutputs & m_eCommServiceType)){
    ...
    generateGenericDataPointArrays("IN", &(doDataTypeNames[2]), &(doNames[2]), paInterfaceSpec.m_nNumDOs - 2);
  }
  else{
    doDataTypeNames = new CStringDictionary::TStringId[paInterfaceSpec.m_nNumDOs];
    doNames = new CStringDictionary::TStringId[paInterfaceSpec.m_nNumDOs];
...
EComResponse CCommFB::receiveData() {
  ...
  DEVLOG_DEBUG("----------------EComResponse CCommFB::receiveData()------------------\n");
