...
void CGenFunctionBlock<T>::generateGenericInterfacePointNameArray(const char * const paPrefix, CStringDictionary::TStringId* paNamesArayStart,
  ...
  DEVLOG_DEBUG("----------------genfb.tpp generateGenericInterfacePointNameArray noOfDigits = %i------------------\n", noOfDigits);
  DEVLOG_DEBUG("----------------genfb.tpp generateGenericInterfacePointNameArray paPrefix = %s------------------\n", paPrefix);
  ...
  if(cg_nIdentifierLength >= (len + noOfDigits)){
    ...
    for(size_t i = 1; i <= paNumGenericDataPoints; i++) {
      if(i < 10){ //1 digit
        ...
        acBuffer[len] = '\0';
        //DEVLOG_DEBUG("----------------genfb.tpp generateGenericInterfacePointNameArray 1digit acBuffer = %c------------------\n", acBuffer[len]);
      }
