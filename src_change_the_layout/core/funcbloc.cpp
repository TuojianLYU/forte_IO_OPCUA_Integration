...
void CFunctionBlock::sendOutputEvent(size_t paEO){
  DEVLOG_DEBUG("----------------funcbloc CFunctionBlock::sendOutputEvent------------------\n");
  ...
  if(paEO < m_pstInterfaceSpec->m_nNumEOs) {
    DEVLOG_DEBUG("----------------funcbloc CFunctionBlock::sendOutputEvent (paEO < m_pstInterfaceSpec->m_nNumEOs)------------------\n");
    if(0 != m_pstInterfaceSpec->m_anEOWithIndexes && -1 != m_pstInterfaceSpec->m_anEOWithIndexes[paEO]) {
      DEVLOG_DEBUG("----------------funcbloc CFunctionBlock::sendOutputEvent (0 != m_pstInterfaceSpec->m_anEOWithIndexes && -1 != m_pstInterfaceSpec->m_anEOWithIndexes[paEO])------------------\n");
      ...
      for(size_t i = 0; eiWithStart[i] != scmWithListDelimiter; ++i) {
        DEVLOG_DEBUG("----------------funcbloc CFunctionBlock::sendOutputEvent inside for loop------------------\n");
        CDataConnection *con = getDOConUnchecked(eiWithStart[i]);
        if(con->isConnected()) {
          DEVLOG_DEBUG("----------------funcbloc CFunctionBlock::sendOutputEvent inside if con->isConnected()------------------\n");
